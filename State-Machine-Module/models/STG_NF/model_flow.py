...
import sys
sys.path.append("D:\\project_python\\STG-NF")
from models.STG_NF.modules_flow import (
    Conv2d,
    Conv2dZeros,
    ActNorm2d,
    InvertibleConv1x1,
...
class FlowStep(nn.Module):
    ...
    def normal_flow(self, input, logdet):
        ...
        # 1. actnorm
        z, logdet = self.actnorm(input, logdet=logdet, reverse=False)  # bsx 2 x 24 x 18
...
class FlowNet(nn.Module):
    ...
    def encode(self, z, logdet=0.0):
        logdet = torch.zeros(z.shape[0]).to(self.device) # 256
...
class STG_NF(nn.Module):
    ...
    def normal_flow(self, x, label, score):
        b, c, t, v = x.shape # 256 2 24 18

        z, objective = self.flow(x, reverse=False) # 256 2 24 18
        # objective = 256
        mean, logs = self.prior(x, label) # lable 均为1 正常 无监督的训练
        objective += gaussian_likelihood(mean, logs, z)

        # Full objective - converted to bits per dimension
        nll = (-objective) / (math.log(2.0) * c * t * v) # 256
...
if __name__ == '__main__':
    ...
    model.cuda().set_actnorm_init()
    x = torch.rand(256, 2, 24, 18).cuda()
    z, nll = model(x)
    print(z.shape) # 256 4 12 18
    print(nll.shape) # 256 
    x_recon = model(z, reverse=True)
    print(x_recon.shape)
    print(x_recon)
    print(x)
    print(torch.mean((x_recon - x) ** 2))
    print(torch.mean((x_recon - x) ** 2) / torch.mean(x ** 2))
    print(torch.mean((x_recon - x) ** 2) / torch.mean(x ** 2) * 100)