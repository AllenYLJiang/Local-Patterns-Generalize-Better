...
def compute_loss(nll, reduction="mean", mean=0):
    if reduction == "mean":
        losses = {"nll": torch.abs(torch.mean(nll))}
...
class Trainer:
    ...
    def train(self, log_writer=None, clip=100):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, batch in enumerate(pbar):
                try:
                    #### 异常数据label 为 1， 正常为 0 ####
                    flows_batch, flows_batch_idx = batch
                    # data = [data.to(self.args.device, non_blocking=True) for data in data_arr]
                    flows_batch = flows_batch.to(self.args.device, non_blocking=True)
                    samp = flows_batch.squeeze(1)
                    label = torch.ones(samp.shape[0]).to(self.args.device, non_blocking=True) ## 1 为正常
                    # if self.args.model_confidence:
                    #     samp = flows_batch[0]
                    # else:
                    #     samp = flows_batch[0][:, :2]
                    z, nll = self.model(samp.float(), label=label, score=1)
                    if nll is None:
                        continue
                    # if self.args.model_confidence:
                    #     nll = nll * score
            ...
            self.save_checkpoint(epoch, filename="ep"+ str(epoch) + "_" +checkpoint_filename)
    ...
    def test(self):
        ...
        for itern, batch in enumerate(pbar):
            ...
            #     samp = data[0][:, :2]
            with torch.no_grad():
                z, nll = self.model(samp.float(), label=torch.ones(samp.shape[0]), score=1)
            # if self.args.model_confidence:
            #     nll = nll * score
