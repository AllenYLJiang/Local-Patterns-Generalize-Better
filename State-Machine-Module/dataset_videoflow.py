...
class VideoDatasetWithFlows(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.train: ## track id
        #     return img_batch, flows_batch, torch.zeros(1),object_id
        # return img_batch, flows_batch, self.all_gt[index], object_id

        ### train 先随机选一个用也行吧 ？
        if self.train: ## track id
            indices = np.random.choice(len(img_batch), 1, replace=False)
            return img_batch[indices], flows_batch[indices], torch.zeros(1),object_id
            # return img_batch, flows_batch, torch.zeros(1),object_id
        return img_batch, flows_batch, self.all_gt[index], object_id
...
if __name__ == '__main__':
    ...
    for i, (img, flows, gt, object_id) in enumerate(loader["train"]):
        print(img.shape, flows.shape, gt.shape, object_id)
