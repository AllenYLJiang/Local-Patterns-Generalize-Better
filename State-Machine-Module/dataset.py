...
from utils_r.pose_utils import gen_clip_seg_data_np, get_ab_labels
...
class PoseSegDataset(Dataset):
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
        ...
        self.seg_len = dataset_args.get('seg_len', 34)
        ...
        # self.num_samples, self.T,self.C,  self.V = self.segs_data_np.shape ## C T 交换
    ...
    def __getitem__(self, index):
        ...
        if self.normalize_pose_segs:
            ...
            data_transformed = normalize_pose(data_transformed.transpose((1, 2, 0))[None, ...],
                                              **self.args)[0].squeeze(axis=0).transpose(2, 0, 1)
            ...
            add_offsets = [[data_transformed_earliest_box_size * x[0], data_transformed_earliest_box_size * x[1]] for x in sequential_boxes_offsets_divided_by_box_size]
            data_transformed[0, :, :] = np.array([(data_transformed[0, x, :] + np.array(add_offsets)[x, 0]) for x in range(len(data_transformed[0, :, :]))])
            data_transformed[1, :, :] = np.array([(data_transformed[1, x, :] + np.array(add_offsets)[x, 1]) for x in range(len(data_transformed[1, :, :]))])

        ret_arr = [data_transformed, trans_index]

        # ######## great ########### 0706 ck
        # self.segs_data_np = self.segs_data_np
        # ####################
    ...
    def get_all_data(self, normalize_pose_segs=True):
        if normalize_pose_segs:
            segs_data_np = normalize_pose(self.segs_data_np.transpose((0, 2, 3, 1)), **self.args)[0].transpose(
                (0, 3, 1, 2))
        else:
...
def get_dataset_and_loader(args, trans_list, only_test=False):
    ...
    # splits = ['train'] if not only_test else ['test']
    # splits = ['test']
    for split in splits:
        evaluate = split == 'test'
...
def gen_dataset(person_json_root, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True,
    ...
    seg_len = dataset_args.get('seg_len', 34)
    ...
    # if normalize_pose_segs:
    #     segs_data_np = normalize_pose(segs_data_np, vid_res=vid_res, **dataset_args)[0]
    #     global_data_np = normalize_pose(global_data_np, vid_res=vid_res, **dataset_args)[0]
    #     global_data = [normalize_pose(np.expand_dims(data, axis=0), **dataset_args)[0].squeeze() for data
    #                    in global_data]
    ...
    segs_data_np = np.transpose(segs_data_np, (0, 3, 1, 2)).astype(np.float32) # 24 18 3 ---> 3 24 18
