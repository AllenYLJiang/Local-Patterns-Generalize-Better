...
def normalize_pose_v2(pose_data, **kwargs):
    ...
    # per moment normalization
    std_list, return_mean, return_std = [], [], []
    for instant_idx in range(pose_data_centered.shape[1]):
        std_list.append(pose_data_centered[:, instant_idx, :, 1].std(axis=1)[0])
        return_mean.append(pose_data_centered[:, instant_idx, :, :2].mean(axis=1)[:, None, :])
        return_std.append(pose_data_centered[:, instant_idx, :, 1].std(axis=1)[0])
        pose_data_centered[:, instant_idx, :, :2] = (pose_data_centered[:, instant_idx, :, :2] - pose_data_centered[:, instant_idx, :, :2].mean(axis=1)[:, None, :]) / pose_data_centered[:, instant_idx, :, 1].std(axis=1)[0]
    for instant_idx in range(1, pose_data_centered.shape[1]):
        pose_data_centered[:, instant_idx, :, :2] = pose_data_centered[:, instant_idx, :, :2] * std_list[instant_idx] / std_list[0]
    pose_data_zero_mean = pose_data_centered

    # return pose_data_zero_mean
    # return_mean, return_std = pose_data_centered[..., :2].mean(axis=(1, 2)), pose_data_centered[..., 1].std(axis=(1, 2))
    ...
    return pose_data_zero_mean, norm_factor, return_mean, return_std
